...
For every completed prompt-based report, the app fetches two MCP evidence items before it asks the LLM to write: **tennis** uses `Get_Match_Stats` + `Get_Match_Point_by_Point`; **football** asks for `Get_Match_Summary` + `Get_Match_Stats`. If the subscribed RapidAPI catalog does not expose or allow `Get_Match_Summary`, the server safely falls back to `Get_Match_Details`. Keep the four existing tool names in `RAPID_MCP_ALLOWED_TOOLS`, and add `Get_Match_Summary` when it appears in `GET /api/mcp/tools`; failed MCP calls remain a soft fallback so the existing verified API evidence can still produce a report.
