...
test("MCP enrichment: enriches every completed football match within the cap", () => {
  ...
  const requests = selectMcpRequests(match);
  assertEq(requests.length, 3, "three bounded tool calls");
  assertEq(requests[0]?.name, "Get_Match_Details", "details are first for football");
  assertEq(requests[1]?.name, "Get_Match_Summary", "summary is second for football");
  assertEq(requests[2]?.name, "Get_Match_Commentary", "commentary is third for football");
  assertEq(requests[2]?.optional, true, "commentary is skipped safely when unavailable");
  assertEq(requests[0]?.arguments.match_id, match.id, "match id passed to MCP");
});
