...
describe("POST /api/matches", () => {
  it("appends row to Tennis tab and returns 202 with row_index", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({ ok: true, json: async () => ({ updates: { updatedRange: "Tennis!A2:B2" } }) })
    );
  ...
  it("routes soccer to Soccer tab with Football tag", async () => {
    stubGoogleEnv();
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ updates: { updatedRange: "Soccer!A3:B3" } }) });
