...
const VALID_CREDS = {
  ...
  expiry: new Date(Date.now() + 3600_000).toISOString(), // còn 1h
...
describe("getAccessToken", () => {
  it("returns existing token when not expired (no network call)", async () => {
    vi.stubGlobal("process", { ...process, env: { GOOGLE_TOKEN_JSON: JSON.stringify(VALID_CREDS) } });
  ...
  it("refreshes via token endpoint when token expired", async () => {
    const expired = {
      ...VALID_CREDS,
      token: "expired-token",
      expiry: new Date(Date.now() - 60_000).toISOString(), // hết hạn 1 phút
    };
    vi.stubGlobal("process", { ...process, env: { GOOGLE_TOKEN_JSON: JSON.stringify(expired) } });
    ...
    // Kiểm tra body gửi đúng grant_type + refresh_token
  ...
  it("throws when refresh endpoint returns non-ok", async () => {
    const expired = { ...VALID_CREDS, expiry: new Date(Date.now() - 60_000).toISOString() };
    vi.stubGlobal("process", { ...process, env: { GOOGLE_TOKEN_JSON: JSON.stringify(expired) } });
